...
# Load the data
confirmed <- read_csv("C:\\Users\\cuate\\Documents\\CodAcG_Github\\R_Projects\\CoronavirusProject\\corona_confirmed.csv")
recovered <- read_csv("C:\\Users\\cuate\\Documents\\CodAcG_Github\\R_Projects\\CoronavirusProject\\corona_recovered.csv")
deaths <- read_csv("C:\\Users\\cuate\\Documents\\CodAcG_Github\\R_Projects\\CoronavirusProject\\corona_deaths.csv")
...
conf_ne <- confirmed %>% filter(Lat > 0) %>% select("3/22/20") %>% sum()
conf_ne
# Filter for Australia cases
conf_au <- confirmed %>% filter(`Country/Region` == "Australia") %>% select("3/22/20") %>% sum()
conf_au
# Display percentages
print(conf_ne / total_cases)
print(conf_au / total_cases)
```
...
```{r}
# Group by countries
conf_by_contry <- confirmed %>% group_by(`Country/Region`) %>% summarize_at(vars(-Lat, -Long, -`Province/State`), sum)
conf_by_contry
# Filter the grouped data set for Australia
conf_au <- conf_by_contry %>% filter(`Country/Region` == "Australia") %>% select("3/22/20") %>% sum()
conf_au
```
...
```{r}
# Group by countries
rec_by_contry <- recovered %>% group_by(`Country/Region`) %>% summarize_at(vars(-Lat, -Long, -`Province/State`), sum)
rec_by_contry
# Filter the grouped data set for US
conf_us <- conf_by_contry %>% filter(`Country/Region` == "US") %>% select("3/22/20")
rec_us <- rec_by_contry %>% filter(`Country/Region` == "US") %>% select("3/22/20")
print(rec_us / conf_us * 100)
```
...
rec_us <- rec_by_contry %>% filter(`Country/Region` == "US")
rec_us
```
...
max_conf_us <- conf_by_contry %>% filter(`Country/Region` == "US") %>% select(-(`Country/Region`)) %>% max()
max_rec_us <- rec_by_contry %>% filter(`Country/Region` == "US") %>% select(-(`Country/Region`)) %>% max()
max_conf_us
max_rec_us
print(max_rec_us/max_conf_us*100)
```
...
confirmed_trans <- conf_by_contry %>% t() %>% as.data.frame()
head(confirmed_trans)
```
...
confirmed_trans <- confirmed_trans %>% row_to_names(row_number = 1)
head(confirmed_trans)
```
...
To do so, pass your data frame to the [`apply()`](https://www.guru99.com/r-apply-sapply-tapply.html) function. `apply()` takes two parameters in addition to the data set that you're using. The `MARGIN` parameter should be set to `2` — this tells `apply()` to work with columns rather than rows.
...
confirmed_trans <- confirmed_trans %>% apply(MARGIN = 2, as.numeric) %>% as.data.frame()
head(confirmed_trans)
```
...
```{r}
# Add the date column
confirmed_trans <- confirmed_trans %>% mutate(date = c(1:nrow(confirmed_trans)))
head(confirmed_trans)
...
confirmed_trans %>%
  ...
  geom_line(aes(y=China), color="darkgreen") +
  labs(
    title = "Covid Cases US & China",
    subtitle = "Data until March 23rd",
    x = "Days",
    y = "Confirmed Cases"
  )
```
...
#Recovered
recovered_trans <- rec_by_contry %>% t() %>% row_to_names(row_number = 1) %>% apply(MARGIN =2, as.numeric) %>% as.data.frame
recovered_trans <- recovered_trans %>% mutate(date = c(1:nrow(recovered_trans)))
head(recovered_trans)

#Deaths
death_by_contry <- deaths %>% group_by(`Country/Region`) %>% summarize_at(vars(-Lat, -Long, -`Province/State`), sum)

deaths_trans <- death_by_contry %>% t() %>% row_to_names(row_number = 1) %>% apply(MARGIN = 2, as.numeric) %>% as.data.frame()
deaths_trans <- deaths_trans %>% mutate(date = c(1:nrow(recovered_trans)))
head(deaths_trans)

# Recovered & Death plots
library(gridExtra)

plot1 <- confirmed_trans %>%
  ggplot(aes(x = date)) +
  geom_line(aes(y = US), color = "blue") +
  geom_line(aes(y = China), color = "darkgreen") +
  labs(

...
temp_annecy_f <- c(79,80,78,71,74,65,64)
temp_annecy_f
```
...
temp_annecy_c <- far_to_cel(temp_annecy_f)
temp_annecy_c
```
...
```{r}
# Write your function here
check_threshold <- function(temps) {
  threshold <- length(temps)
  count <- 0
  for (i in temps) {
      if (i > 60) {
      count <- count + 1
...
temp_tokio_f <- c(76,77,81,84,81,78,78)
temp_prince_f <- c(66,70,75,78,71,67,61)
```
...
check_threshold(temp_tokio_f)
check_threshold(temp_prince_f)
...
df <- data.frame(temp_annecy_f,temp_prince_f,temp_tokio_f)
df
df_longer <- pivot_longer(
  data = df,
  cols = c(temp_annecy_f, temp_prince_f, temp_tokio_f),
  names_to = "temp_cities",
  values_to = "temp"
)
df_longer

plot <- ggplot(data=df_longer, aes(x = temp_cities, y = temp, color=temp_cities))+
  geom_boxplot() +
  labs(
    title = "Temp in possible cities",
    x = "Cities", y = "Temperature"
  )
plot
```
